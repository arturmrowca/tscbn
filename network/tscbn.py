...
import random
from _include.visual.graph_plotter import Visualizer as V
import copy
import numpy as np
...
class TSCBN(HyBayesianNetwork):
    '''
    This class represents a Temporal State Change Bayesian network with hybrid nodes.
    '''
    ...
    def draw(self, mode = "ext", conditions = dict()):
        probs = conditions
        if mode == "ext":
            V().draw_network_graph_given(self.E, [f for f in self.V if not(str.startswith(f, "dL_") or str.startswith(f, "tp_"))], probs, self.Vdata, True)
        if mode == "int":
