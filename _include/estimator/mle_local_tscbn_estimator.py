    ...
    def _satisfies_parent_conditions(self, parent_starts, parent_outcome):
        ...
        for idx in range(len(parent_starts)):
            ...
            try:
                ...
                me_start = int(self._last_symbol[1])
                me_end = int(self._last_symbol[2])
            except:
...
class MLELocalParameterEstimator(ParameterEstimator):
    '''
    Idea:
    ...
    def __init__(self):
        super(MLELocalParameterEstimator, self).__init__()
    ...
    def _estimate_tscbn(self, sequences, debug):
        ...
        for i in range(repeat_frequency):
            ...
            # ------------------------------------------------------------------
            #   1. Draw and do MLE Estimation
            # ------------------------------------------------------------------
            # per sequence draw one sample - do MLE - add to estimate - repeat
            sequence_samples = []

            # DO THIS ONLY IF SEQUENCE IS NOT COMPLETE
            ...
            for seq_count in range(len(sequences)):
                ...
                sequence_samples.append(seq)

            # do MLE over all sequences
            learner = PGMLearner()
            local_dbn_estimate = learner.discrete_mle_estimateparams(skel, sequence_samples)


            # ------------------------------------------------------------------
            #   2. Count MLE Estimation to TSCBN
            #      a. ensure vals have same order
            #      b. ensure conditions have same order
            # ------------------------------------------------------------------
            #("FETTES PROBLEM - geloest: Die reihenfolge der vals ist nicht immer gleich - AUCH BEI CONDITION?! -> d.h. est [a,b,c] und real [b,a,c]")

            for v in local_dbn_estimate.V:

                # I. Check mismatch -> add where ok
                l_learned = len(self.tbn.Vdata[v]["vals"])
                l_est = len(local_dbn_estimate.Vdata[v]["vals"])

                # a. ensure vals are in same order (e.g. est [a,b,c] und real [b,a,c]) -> else rearrange
                rearrange_vals_indices = False
                if l_learned == l_est and self.tbn.Vdata[v]["vals"] != local_dbn_estimate.Vdata[v]["vals"]:
                    rearrange_vals_indices = np.array([local_dbn_estimate.Vdata[v]["vals"].index(x) for x in self.tbn.Vdata[v]["vals"]]) # indices in right order

                # b. ensure conds are in same order -> else rearrange
                rearrange_parents_indices = False
                if self.tbn.Vdata[v]["parents"] != local_dbn_estimate.Vdata[v]["parents"]:
                    rearrange_parents_indices = np.array([self.tbn.Vdata[v]["parents"].index(x) for x in local_dbn_estimate.Vdata[v]["parents"]]) # indices in right order

                # ITERATE OVER CONDITIONED
                if isinstance(self.tbn.Vdata[v]["cprob"], dict):
                    for k in self.tbn.Vdata[v]["cprob"]:

                        # O. Correct order of values
                        if rearrange_parents_indices:
                            k_loc = str(list(np.array(eval(k))[rearrange_parents_indices])) # rearrange e.g. k = [a,b] and k_loc = [b,a] as parents sorted other way round
                        else:
                            k_loc = k

                        # then for this condition no measurement was found -> skip
                        if not k_loc in local_dbn_estimate.Vdata[v]["cprob"]:
                            continue

                        # I. Check mismatch -> add where ok
                        #l_learned = len(self.tbn.Vdata[v]["cprob"][k])
                        #l_est = len(local_dbn_estimate.Vdata[v]["cprob"][k_loc])
                        if l_learned == l_est:
                            # Correct order of values
                            if self.tbn.Vdata[v]["vals"] != local_dbn_estimate.Vdata[v]["vals"]:
                                rel_array = np.array(local_dbn_estimate.Vdata[v]["cprob"][k_loc])[rearrange_vals_indices]
                                self.tbn.Vdata[v]["cprob"][k] += rel_array

                        else:
                            # II. Mismatch -> Found
                            # get indices matching
                            indices = np.array([self.tbn.Vdata[v]["vals"].index(x) for x in local_dbn_estimate.Vdata[v]["vals"]])
                            self.tbn.Vdata[v]["cprob"][k][indices] += local_dbn_estimate.Vdata[v]["cprob"][k_loc]

                else:
                    # ITERATE OVER Leaf Nodes
                    # mismatch -> add where ok
                    #l_learned = len(self.tbn.Vdata[v]["cprob"])
                    #l_est = len(local_dbn_estimate.Vdata[v]["cprob"])

                    if l_learned == l_est:
                        # Correct order of values
                        if self.tbn.Vdata[v]["vals"] != local_dbn_estimate.Vdata[v]["vals"]:
                            rel_array = np.array(local_dbn_estimate.Vdata[v]["cprob"])[rearrange_vals_indices]
                            self.tbn.Vdata[v]["cprob"] += rel_array

                    else:
                        # II. Mismatch -> Found
                        # get indices matching
                        indices = np.array([self.tbn.Vdata[v]["vals"].index(x) for x in local_dbn_estimate.Vdata[v]["vals"]])
                        self.tbn.Vdata[v]["cprob"][indices] += local_dbn_estimate.Vdata[v]["cprob"]
    ...
    def draw_valid_sample(self, initial_states, trees, delta_t_distribution):
        ...
        while node_set:
            ...
            if n.split("_")[-1] == "0":
                ...
                val = initial_states[self._tv(n)][0]
                delta_t_distribution["dL_" + n] = {}
                t_abs[n], t_abs_end[n], sequence[n] = 0.0, initial_states[self._tv(n)][2], val
                ...
                val = trees[self._tv(n)].get_next_symbol(parent_starts, self._parent_outcome(n, sequence), cond)
                if val is None:
        ...
        for tv_sequence in sequences:
            ...
            for n in list(self.tbn.nodes.keys()):
                ...
                if not tv in trees:
                    ...
                    rel_nodes[tv] = [n for n in list(self.tbn.nodes.keys()) if self._tv(n) == tv][1:] # ACHTUNG V1 includes V11, V12 ... NO!
    ...
    def _single_run(self, initial_states, trees, seq_count, len_sequences, debug, disable_out = True):
        ...
        for tz in range(self.sampling_frequency):
            ...
            while node_set:
                ...
                if n.split("_")[-1] == "0":
                    ...
                    # DRAW LEAF NODE INITIAL SAMPLE
                    val = initial_states[self._tv(n)][0] #L().log.debug("%s - I return: %s " % (str(n), str(val)))
                    current_sample_initial.append([n, pars[n]["tbn_vals"].index(val)])  # info, info
                    delta_t_distribution["dL_" + n] = {}
                    if self._debug_time: trees[self._tv(n)].delta_t_for_debug["dL_" + n] = {}
                    if self._debug_time: trees[self._tv(n)].delta_t_for_debug["dL_" + n][str([val])] = 0

                    t_abs[n], t_abs_end[n], delta_t_distribution["dL_" + n][str([val])], set_values[n] = 0.0, initial_states[self._tv(n)][2], [0.0], val
                    ...
                    val = trees[self._tv(n)].get_next_symbol(parent_starts, self._parent_outcome(n, set_values), cond)
                    ...
                    # DEBUG - ONLY HERE!!!!!!!!!
                    if self._debug_time:
                        if "dL_" + n  not in trees[self._tv(n)].delta_t_for_debug:
                            trees[self._tv(n)].delta_t_for_debug["dL_" + n] = {}
                        if not str(cond_dL) in trees[self._tv(n)].delta_t_for_debug["dL_" + n]:
                            trees[self._tv(n)].delta_t_for_debug["dL_" + n][str(cond_dL)] = []  # Summe, Anzahl
        ...
        for k in delta_t_distribution:
            for j in delta_t_distribution[k]:
                ...
                if self._debug_time: trees[self._tv(k.replace("dL_", ""))].delta_t_for_debug[k][j] = {'variance': var, 'mean_base': mean}
    ...
    def _is_never(self, node_name, set_vals):
        ...
        if np.all(np.array(set_vals[node_name]) == np.array(set_vals[self._tv(node_name)+"_"+str(parent_idx)])):
            return True
        return False

    def _parent_outcome(self, node_name, set_vals):
        parent_idx = int(node_name.split("_")[-1]) - 1
        return  set_vals[self._tv(node_name)+"_"+str(parent_idx)]
