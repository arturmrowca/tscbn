...
class VariationalInferenceOptimizedParameterEstimator(ParameterEstimator):
    '''
    Variational Inference Algorithm
    '''

    def __init__(self):
        super(VariationalInferenceOptimizedParameterEstimator, self).__init__()
        ...
        self._relevant_outcome_combos_time = dict()
    ...
    def _recompute_global_parameters(self, sequences, start_time):
        '''
        ...
        self._update_cpd(sequences)
        L().log.info("----> Recomputing continuous CPDs")
        self._update_times(sequences)
    ...
    def _estimate_tscbn(self, sequences, debug):
        ...
        for vi_iteration in range(self.iteration_frequency):
            ...
            self._new_global_cpd = dict()
            self._new_global_cpd_t = dict()
            self._new_mu_times_q = dict()
            self._new_sum_q = dict()

            start_time, k = self._initialize_vi_iteration(vi_iteration)

            #  4. Compute CAVI Update per sequence
            for sequence in sequences:
                k = self._update_sequence(k, sequence)

                # Compute CAVI Update
                self._variational_iteration(sequence, k) # each observation adds to the new tree

            #Compute global parameters
            self._recompute_global_parameters(sequences, start_time)

        self._log(3)

    def _update_times(self, sequences):
        '''
            ...
            for cond in self.tbn.Vdata[node]["hybcprob"]:
                ...
                for k in range(len(sequences)):
                    ...
                    for pa in self.tbn.Vdata[node]["parents"]:
                        i += 1
                        q *= self._q_latent[k][pa][self.tbn.Vdata[pa]["vals"].index(eval(cond)[i])] # this seq: q(pa) * mu(pa)

                    # Add outcomes to sum
                    mu = self._q_latent_t[k][node]["mean_base"]
                    sum_mu_times_q += mu * q
                    sum_q += q
    ...
    def _update_cpd(self, sequences):
        '''
        ...
        for node in self.tbn.V:
            ...
            if self.tbn.Vdata[node]["parents"] is None:
                ...
                self.tbn.Vdata[node]["cprob"] = np.mean(np.array([n[node] for n in self._q_latent.values()]), axis=0)
            ...
            for k in range(len(sequences)):
                ...
                for cond in self.tbn.Vdata[node]["cprob"]:
                    ...
                    p = copy.deepcopy(self._q_latent[k][node]) # node estimate
                    i = -1
                    for pa in self.tbn.Vdata[node]["parents"]: # parent estimates
                        i += 1
                        p *= self._q_latent[k][pa][self.tbn.Vdata[pa]["vals"].index(eval(cond)[i])]
                    new_cprob[cond] += p
