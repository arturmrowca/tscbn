...
class VIElement(object):
    ...
    def __init__(self, seq_group, q_latent, q_latent_t, tbn, rel_outcome_combos):
        ...
        self._large_scale = False # reading from storage
    ...
    def perform_iteration(self):
        ...
        for seq_id in self.seq_ids:
            i += 1
            if i % 20 == 0:
                print("Did %s of %s" % (str(i), str(len(self.seq_ids))))
    ...
    def _update_latent_time(self, node, seq_id, observation):
        ...
        # Load from memory -> then store again
        if self._large_scale:
            destination_file = self.relevant_outcome_combos[seq_id]
            with open(destination_file, 'rb') as in_strm:
                rel_combos = dill.load(in_strm)
        else:
            rel_combos = self.relevant_outcome_combos[seq_id]

        for outcome in rel_combos:
            iter += 1
    ...
    def _update_latent_node(self, seq_id, observation, node, retry, temporal_cond):
        ...
        if self._large_scale and not isinstance(self.relevant_outcome_combos[seq_id], list):
            destination_file = self.relevant_outcome_combos[seq_id]
            with open(destination_file, 'rb') as in_strm:
                rel_combos = dill.load(in_strm)
        else:
        ...
        if isinstance(rel_combos, list): gen = True
...
class VariationalInferenceParallelParameterEstimator(ParameterEstimator):
    ...
    def __init__(self):
        ...
        self.sub_groups_seq_combos = 20 # batch size -> split data into this much batches

        # information
        self._large_scale = False # if true store and load combinations
        self.sub_groups = 100
    ...
    def iter_det_relevant(self, nodes, observations, seq_ids):
        res_dict = dict()
        indices_to_drop = [] # if too latent -> drop
        k = -1
        # for known observations -> return known
        total_size = 0
        for observation in observations:
            k += 1
            seq_id = seq_ids[k]
            res_dict[seq_id], drop = self._determine_relevant_light(seq_id, observation)

            if drop and self._large_scale:
                indices_to_drop.append(seq_id)
                print("---> Dropping " + str(seq_id))

            # if large_scale
            import os
            if self._large_scale and not drop:
                destination_file = os.path.join("store/tmp/", "seq_" + str(seq_id) + ".dill")
                print("Found model - storing to %s" % str(destination_file))
                with open(destination_file, "wb") as dill_file:
                    dill.dump(res_dict[seq_id], dill_file, recurse=True)
                del res_dict[seq_id]
                gc.collect()
                res_dict[seq_id] = destination_file



            #total_size += sys.getsizeof(res_dict[seq_id])

            #print("Size Res-Dict: " + str(total_size) + " Bytes")
        return [res_dict, indices_to_drop]
    ...
    def _estimate_tscbn(self, sequences, debug):
        ...
        if self._large_scale:
            for input in inputs:
                output = self.iter_det_relevant(input[0], input[1], input[2])
                self._relevant_outcome_combos = {**self._relevant_outcome_combos, **output[0]}
                indices_to_drop += output[1]
        # END DEBUG
        else:
            outputs = parallelize_stuff(inputs, self.iter_det_relevant, simultaneous_processes = self._parallel_processes, print_all = True)
        ...
        for i in range(0, len(sequences), sub_groups):
            ...
            rel_indices = list(range(prev, i))
            if self._large_scale:
                rel_indices = [r for r in rel_indices if not r in indices_to_drop]
    ...
    def _update_times(self, iter_outs):
        ...
        p(A|B,C) = q(A)*q(B)*q(C)
        mu = weighted mean of q_all(outcomes) * mu over all sequences = q1*m1 + q2*m2 + q3*m3 / q1 + q2 + q3
        :param sequences: List of sequences in format: list element is dict. of key: TV name value: [state, start_time_of_state, end_time_of_state]
        '''
    ...
    def _determine_relevant_light(self, seq_id, observation):
        ...
        rel_combos = self._only_short_is_original(extended_combos, observation, relevant_outcomes_nodes)

        if len(rel_combos)> 100000:
            print("dropping")
            drop = True
        ...
        return rel_combos, drop

    def _only_short_is_original(self, extended_combos, observation, relevant_outcomes_nodes):
        '''
        For a given set of extended sequences e.g. AABBDD, ABDC determine the ones
        ...
        #for st in zip(*relevant_outcomes_nodes.values()):
        #    o = 0
        tvs = list(set(["_".join(n.split("_")[:1]) for n in relevant_outcomes_nodes.keys()]))
        comb_dic = dict() # things to concat
        for tv in tvs:# ZIEL: V_0:[o_0, 2, 4] V_1:[o_1, 4, 8]
            comb = dict([(k, relevant_outcomes_nodes[k]) for k in relevant_outcomes_nodes.keys() if k.split("_")[0] == tv])
            comb_dic[tv] = []
            # filter for observable
            observed = [a[0] for a in observation[tv]]
            xx = [a for a in (dict(zip(comb, x)) for x in product(*comb.values()))]
            for val in xx:
                short = self._drop_duplicate_list([v[0] for v in val.values()])
                if not np.array_equal(short, observed): continue
                comb_dic[tv] += [val]
        res_lst = [{k: v for d in x for k, v in d.items()} for x in product(*comb_dic.values())]# zlib.compress(dill.dumps([{k: v for d in x for k, v in d.items()} for x in product(*comb_dic.values())]))

        # delete stuff to free memory
        #del observation
        #del relevant_outcomes_nodes
        #del comb
        #del xx
        #del comb_dic
        #gc.collect()
        ...
        """for outc in extended_combos:
            counter += 1
            worked = True
            for n in observation:# for this only observation!
            ...
            if worked:
                res_lst += [outc]
        print("%s vs. %s" % (str(counter), str(len(res_lst))))
        return res_lst"""

    def _only_short_is_original_PREV(self, extended_combos, observation):
        '''
        For a given set of extended sequences e.g. AABBDD, ABDC determine the ones
        that are consistent with the observed e.g. ABC observed, then AB not possible
        :return: list of short and consistent outcome combinations
    ...
    def _log_reference_cpds(self):
        ...
        :param show_latent: if True prints the latent local estimates q(z) as well
        '''
        if self._reference == None:return#no logging
        L().log.info( "---------------------------------------------------------------------------------------------------------------------------------------------------")
